...
export const sendContactMessage = (message, cb) => {
    const emailHtml = generateEmailHtml(message)
    const emailOptions = getEmailOptions(message.name, emailHtml)
...
export const sendMessage = (
    { message = "", sender = "Anonymous", subject },
    cb
) => {
    const emailOptions = {
        from: sender,
...
const getEmailOptions = (sender, html) => ({
    from: sender + "<" + MAILGUN_USER + ">", // sender address
