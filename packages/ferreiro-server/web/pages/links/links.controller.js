...
const getLatestVideo = () => {
	return new Promise((resolve, reject) => {
		try {
			const cachedLatestVideo = cache.get('youtube-latest-video');
			...
			if (!cachedLatestVideo) {
				return axios.get(YOUTUBE_API_ENDPOINT).then((youtubeResponse) => {
					...
					cache.put('youtube-latest-video', latestVideo, CACHED_TIME_MS)
...
export const getLinksPage = async (_, res) => {
	try {
		...
		const projects = [
			{
				url: 'https://ferreiro.me/?utm_source=links&utm_medium=cta&utm_campaign=links-ferreiro',
				title: 'Ferreiro.me',
				image: '/links_jorge_ferreiro_project.jpg',
			},
			{
				url: 'https://www.developersindepth.com/?utm_source=links&utm_medium=cta&utm_campaign=links-ferreiro',
